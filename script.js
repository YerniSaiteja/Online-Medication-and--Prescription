...
loginForm.addEventListener('submit', function (e) {
    ...
    if (isSignInMode) {
        ...
        if (emailValue === 'admin@medicare.com' && passwordValue === 'admin123') {
            console.log('Static Admin Sign In Bypass');
            const adminData = {
                id: 0,
            ...
            showSuccessMessage('Admin sign in successful! Redirecting...', function () {
        ...
        fetch(window.getApiUrl('auth/login'), {
            ...
            .then(data => {
                ...
                if (role === 'doctor') {
                    ...
                    target = 'pharmacist-dashboard.html';
                } else if (role === 'admin') {
        ...
        if (selectedRole === 'admin') {
            showErrorMessage('Admin accounts are restricted and cannot be created via registration.');
            return;
        }
        ...
        fetch(window.getApiUrl('auth/register'), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            ...
            .then(data => {
                ...
                const role = (userData.role || 'patient').toLowerCase();
                let targetPage = 'patient-dashboard.html';

                if (role === 'doctor') {
                    targetPage = 'doctor-dashboard.html';
                } else if (role === 'pharmacist') {
                    targetPage = 'pharmacist-dashboard.html';
                } else if (role === 'admin') {
                    targetPage = 'admin-dashboard.html';
                }

                showSuccessMessage('Account created successfully! Redirecting...', function () {
