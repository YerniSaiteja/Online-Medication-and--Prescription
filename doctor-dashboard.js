...
async function loadPatients() {
    ...
    try {
        const response = await fetch(window.getApiUrl('users'));
        ...
        tableBody.innerHTML = patients.map(patient => {
            ...
            const formattedId = patient.id.toString().padStart(5, '0');
            const age = patient.medicalHistory ? '—' : '—'; // We don't have DOB easily accessible in simple view yet, or can calc from DOB
            // Simplified view
            return `
                <tr>
                    <td>${formattedId}</td>
                    ...
                    <td>${patient.phone || '—'}</td>
                    <td>${patient.medicalHistory ? 'Has History' : 'None'}</td>
                    <td>${new Date(patient.createdAt).toLocaleDateString()}</td>
                    <td><button class="btn-secondary btn-small view-record-btn" onclick="selectPatientForPrescription('${formattedId}', '${patient.fullName.replace(/'/g, "\\'")}')">Prescribe</button></td>
                </tr>
            `;
        }).join('');
...
// Helper to prefill prescription form
window.selectPatientForPrescription = function (id, name) {
    ...
    // Switch tab
    const tab = document.querySelector('[data-page="create-prescription"]');
    if (tab) tab.click();
...
async function loadMedicineOptions() {
    ...
    try {
        // Fetch all inventory
        const response = await fetch(window.getApiUrl('inventory'));
        if (response.ok) {
...
function initCreatePrescriptionForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        // Validate Patient ID (numeric check)
        if (!/^\d+$/.test(patientId)) {
            showNotification('Patient ID must be a number.');
            submitBtn.textContent = originalBtnText;
            submitBtn.disabled = false;
            return;
        }
        ...
        try {
            const response = await fetch(window.getApiUrl('prescriptions'), {
                method: 'POST',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadPatients(); // Load dynamic patients
...
async function loadPrescriptionHistory() {
    ...
    try {
        const response = await fetch(window.getApiUrl(`prescriptions?doctor_id=${userData.id}`));
