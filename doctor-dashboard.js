...
function initCreatePrescriptionForm() {
    ...
    if (clearFormBtn) {
        clearFormBtn.addEventListener('click', () => {
            ...
            document.getElementById('doseFrequency').value = '';
            document.getElementById('notesInstructions').value = '';
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (!patientName && !medicineName && !doseFrequency && !notesInstructions && !startDate && !endDate) {
            showNotification('No medicines are added');
            return;
        }

        // Check required fields
        if (!patientName || !medicineName || !doseFrequency || !startDate) {
        ...
        // Store medication in localStorage for tracking
...
function downloadPrescriptionFromRow(row) {
...
Generated via MediCare Doctor Portal`;

    const blob = new Blob([content], { type: 'text/plain' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `${patient.replace(/\s+/g, '_')}_${medication.replace(/\s+/g, '_')}.txt`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Attach download handler for initial rows
    document.querySelectorAll('.download-prescription').forEach(btn => {
