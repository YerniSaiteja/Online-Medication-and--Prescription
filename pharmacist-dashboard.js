...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadInventory() {
        try {
            const response = await fetch(window.getApiUrl(`inventory?pharmacist_id=${userData.id}`));
    ...
    window.deleteItem = async (id) => {
        ...
        try {
            await fetch(window.getApiUrl(`inventory/${id}`), { method: 'DELETE' });
            loadInventory();
        } catch (e) {
            console.error(e);
    ...
    async function loadOrders() {
        ...
        try {
            ...
            const response = await fetch(window.getApiUrl('prescriptions?status=Active'));
    ...
    window.dispenseOrder = async (orderId) => {
        ...
        try {
            const response = await fetch(window.getApiUrl(`prescriptions/${orderId}/dispense`), {
                method: 'POST'
            });
