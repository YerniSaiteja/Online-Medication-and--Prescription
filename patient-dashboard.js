...
reminderForm.addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch(window.getApiUrl('reminders'), {
            method: 'POST',
            headers: {
...
async function loadReminders() {
    ...
    try {
        const response = await fetch(window.getApiUrl(`reminders?user_id=${userData.id}`));
...
async function deleteReminder(id) {
    ...
    try {
        const response = await fetch(window.getApiUrl(`reminders/${id}`), {
            method: 'DELETE'
        });
        if (response.ok) {
...
async function loadPrescriptions() {
    ...
    try {
        const response = await fetch(window.getApiUrl(`prescriptions?patient_id=${userData.id}`));
